...
def test3speakers(wav_txt, remote_path, n):
    """
    ...
    for i in range(int(n)):
        ...
        if candidates1 in register100:
            ...
            register100.remove(candidates2)
            candidates3 = sample(register100, 1)[0]  # 从另外98人中抽1人，定为候选人3号
            candidates3_list.append(candidates3)
        else:
        ...
        result = os.popen('curl "http://192.168.99.214:19001/recognize_test?filename=' + remote_path + test_wav +
                          "&ranges=" + candidates1 + '|' + candidates2 + '|' + candidates3 + '"')  # 测试命令
        res1 = result.read()
        print(res1)
        end_time = datetime.datetime.now()  # 测试结束

        if res1 == 'None' or len(res1) == 0:  # 返回为空
            ...
            try:
                ...
                score3 = round(float(res1.split('|')[3]), 3)
                ...
                print(e)
                print("Score Error. Return: %s" % res1)
                return

        real_result_list.append(res1)
        ...
        register100.append(candidates1)
        register100.append(candidates2)
    ...
    dit = {'wav': wav_list, 'actual_name': candidates1_list, 'predicted_name': real_result_list,
           ...
           'score3': score3_list, 'time': during_time, 'null_number': count_null, 'total_time': total_time_list,
           'acc1': acc_list, 'acc2': acc_null_list}
    file_path = r'3speakers_' + str(n) + '_' + excel_name + '.xlsx'
    writer = pd.ExcelWriter(file_path)
    df = pd.DataFrame(dit)
    df.to_excel(writer, columns=['wav', 'actual_name', 'predicted_name', 'candidate1', 'score1', 'candidate2', 'score2',
                                 'candidates3', 'score3', 'time', 'null_number', 'total_time', 'acc1', 'acc2'],
                index=False, encoding='utf-8', sheet_name='3speakers_' + wav_txt[:-4] + '_'+str(n), float_format='%.6f')
    writer.save()
...
if __name__ == '__main__':
    ...
    if not str(times).isdigit():
        ...
        sys.exit()
    test3speakers(wav_txt, remote_path, times)
